...
REQUIREMENTS:
- Total word count: 100-150 words (STRICT - must be concise)
- Structure: 2-3 well-developed paragraphs
- Content: Synthesize across ALL papers - not individual summaries
- Include: Research landscape, major findings, and current state
...
Write a unified synthesis that shows how these papers collectively advance the field:"""
        ...
        yield from self.llm_backend.generate_stream(prompt, max_tokens=3000)
...
- Identify 2-3 major research gaps only
- Total word count: 100-150 words (STRICT - must be concise)
